        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {bookings.map((b) => (
            <div key={b.id} className="border border-gray-200 rounded-xl shadow-md p-5 bg-white hover:shadow-lg transition duration-300">
              ...
              <p className="text-xs text-gray-500 mt-2">Booked on: {new Date(b.createdAt).toLocaleString()}</p>
              <button onClick={() => downloadTicket(b)} className="mt-3 text-sm text-green-600 font-medium hover:underline cursor-pointer">
